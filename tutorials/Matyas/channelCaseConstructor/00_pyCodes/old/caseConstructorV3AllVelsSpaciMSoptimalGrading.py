...
# I/O DATA
#-----------------------------------------------------------------------
baseCase = '../10_baseCaseVSpaciV3/'
outFolder= '../20_toAltix/dynamical/'
name = 'TestSpaci'
name = 'V3dynSpaci'
...
nChX= 2                                                                 #number of channels in hor. dir.
...
# length dimensions
LBf  = 2.0e-3                                                           #length of buffer in front of channels
LPl  = 1.0e-3                                                           #plug length
LCh  = 5.0e-3                                                     	#channel length (total, including the plugs, but WITHOUT the buffers)
...
# MESH DATA
#-----------------------------------------------------------------------
# dX,dY,dZ   = 10e-6,10e-6,10e-6					#domain discretization
dX,dY,dZ   = 19e-6,19e-6,19e-6					#domain discretization
# dX,dY,dZ   = 60e-6,60e-6,90e-6
# dX,dY,dZ   = 50e-6,50e-6,100e-6
# dX,dY,dZ   = 100e-6,100e-6,100e-6
# dX,dY,dZ   = 60e-6,60e-6,80e-6
...
for fraction in fracLst:
    for Vcap in VcapLst:
        ...
        pt1 = [1*WCh+WWl+radius[1]/2.0**0.5+-edgS.verPoint2[0]-WWl/2+offset[0],
               1*WCh+WWl+radius[1]/2.0**0.5+edgS.verPoint2[1]-WWl/2+offset[1],
               LBf+fraction*LCh]
        ...
        nCZLst = [int(round(LLst[0]/dZ)),int(round(LLst[1]/dZ)),int(round(LBf/dZ))]


        
        # mesh grading (basic)
        grX, grY, grZ = 1,1, [0.7,1,40]
        ...
        for j in range(nChY*len(WLstY)):
            for k in range (nChX*len(WLstX)):
                if funWallZone(j,k) or funCoatZone(j,k):
