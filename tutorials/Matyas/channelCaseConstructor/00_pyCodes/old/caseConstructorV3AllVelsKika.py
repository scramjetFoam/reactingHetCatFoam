...
from caseConstructorAuxFuncsV3 import *
...
nCoresPNode = 8                                                         #number of cores per node


nCores      = nNodes*nCoresPNode                                        #number of cores to run the case on
startTime   = 0                                                         #simulation startTime
endTime     = 4999                                                      #simulation endTime
wrInt       = 500                                                       #simulation write interval
...
# width dimensions
# width dimensions
WWl = 216e-6                                                            #wall thickness
WCh = 1250e-6*0.5                                                       #channel width (square)
nChX= 1                                                                 #number of channels in hor. dir.
nChY= 1

# length dimensions
LBf  = 5.0e-3                                                           #length of buffer in front of channels
LPl  = 5.0e-3                                                           #plug length
LCh  = 50.5e-3                                                     	#channel length (total, including the plugs, but WITHOUT the buffers)

# length dimensions
# ~ LBf  = 5.0e-3                                                           #length of buffer in front of channels
# ~ LPl  = 5.0e-3                                                           #plug length
# ~ LCh  = 30.0e-3                                                     	#channel length (total, including the plugs, but WITHOUT the buffers)
...
# CASE PARAMETERS
#-----------------------------------------------------------------------
uInLst    = [                                                           #list with inlet velocities, ms-1
    # ~ 0.51,
#    1.00,
    1.67,
#    2.04,
...
dVals  = [0.062e-12,0.062e-12,0.062e-12]	    		        #porosity values
...
for uIn in uInLst:
    ...
    cLstIO = [0.0,bCI,eCI,bCO,eCO, 1.0]
    ...
    for i in range(len((WLstX))):
        if i%5 == 1:
            nCXLst.append(int(round(WLstX[i]/dX))*nCoatX )
        elif i%5 == 3:
            nCXLst.append(int(round(WLstX[i]/dX))*nCoatX )
        else:
    ...
    for i in range(len((WLstY))):
        if i%5 == 1:
            nCYLst.append(int(round(WLstY[i]/dY))*nCoatX )
        elif i%5 == 3:
            nCYLst.append(int(round(WLstY[i]/dY))*nCoatX )
        else:
    ...
    for j in range(nChY*len(WLstY)):
        for k in range(nChX*len(WLstX)):
            if funBlockNotUsed(j,k,'in'):                                # using imported function (in - inlet plug, out - outlet plug) => return True if not used
    ...
    for j in range(nChY*len(WLstY)):
        for k in range (nChX*len(WLstX)):
            if funCoatZone(j,k,0,bChP):                                        # using imported function => return True if includeds
    ...
    footStr = blockLst[0][0][0].retFileFooterStr()
    for line in footStr:
        bMD.write(line)
    ...
    userMail = user + '@vscht.cz'
    pVals = [userMail,caseName,queName,'%d:ppn=%d'%(nNodes,nCoresPNode),wallTime,"'" + caseName + "'",runScript]
