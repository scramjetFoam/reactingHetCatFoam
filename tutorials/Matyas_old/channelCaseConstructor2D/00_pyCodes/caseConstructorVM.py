...
for i in range(nBlocksX):
    r = 1
    try:
        info = infoBLock[i%nBlocksX]
        if info == 'wall':   r = nTimesWall
        elif 'coat' in info: r = nTimesCoat
    except: 
        pass
    nCXLst.append(WLstX[i]//dX*r)
    if nCXLst[-1] == 0: nCXLst[-1] = 1
...
dXLst = [WLstX[ind]/float(nCXLst[ind]) for ind in range(nBlocksX)]
...
totBlocksX = range(nBlocksX*nChX)  # range of total number of blocks in Xdir
totBlocksY = range(nBlocksY*nChY)  # range of total number of blocks in Ydir
totBlocksL = range(nBlocksL)       # range of total number of blocks in Zdir

blockUse = [[[True for i in totBlocksX] for j in totBlocksY] for l in range(5)]  # True-filled template
    ...
    for j in totBlocksY:
        for k in totBlocksX:
            ...
            if [k%nBlocksX, j%nBlocksY] in mediaInfoDict[mediaName]['notUse'][(nBChx + nBChy + (l-1)//2)%2]:
                blockUse[l][j][k] = False

zones = [[[None for i in totBlocksX] for j in totBlocksY] for l in range(5)]  # None-filled template
    ...
    for j in totBlocksY:
        for k in totBlocksX:
            if blockUse[i][j][k]:
                ...
                if j < nBlocksX*nChY-1 and not blockUse[i][j+1][k]:
    ...
    for j in totBlocksY:
        for k in totBlocksX:
            if blockUse[i][j][k]:
                ...
                if j < nBlocksY*nChY-1 and blockUse[i][j+1][k]:
